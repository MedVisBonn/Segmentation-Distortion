...
#from trainer.ae_trainer import AETrainerACDC
            ...
            #################################################
            ### Temporary Declarations during prototyping ###
            #################################################
        ...
        for layer_id in transformations:
            if layer_id not in self.disabled_ids:
                ...
                hook  = self._get_inspect_transformation_hook(
                    transformations[layer_id], layer_id, n_samples, arch
                )
                self.inspect_transformation_handles[
                    layer_id
                ] = layer.register_forward_pre_hook(hook)
...
def main(args):
    ...
    cfg = {
        'debug': False,
        'log': True,
        'description': f'acdc_AugResDAE{it}_{post}', #'mms_vae_for_nnUNet_fc3_0_bs50',
    ...
    # TODO: Take valid or train transforms. Check in wb for run 8 onwards
    valid_augmentor = batchgenerators.transforms.abstract_transforms.Compose(train_transforms)
    ...
    train_gen = MultiThreadedAugmenter(train_gen, train_augmentor, 2, 4, seeds=None)
    ...
    valid_gen = MultiThreadedAugmenter(valid_gen, valid_augmentor, 2, 4, seeds=None)
